    ...
    public function _saveAction()
    {
		...
		if(isset($data['post'])){
			...
			try {
				...
				$uploader1->addValidateCallback('detailimage', $imageAdapter1, 'validateUploadFile');
				...
				$mediaDirectory1 = $this->_objectManager->get('Magento\Framework\Filesystem')
				                       ->getDirectoryRead(DirectoryList::MEDIA);
				$result1 = $uploader1->save($mediaDirectory1->getAbsolutePath('rokanthemes/blog/images'));
				$data['post']['detailimage'] = 'rokanthemes/blog/images' . $result1['file'];
			} catch (\Exception $e) {
				if ($e->getCode() == 0) {
					$this->messageManager->addError($e->getMessage());
				}
				if (isset($data['detailimage']) && isset($data['detailimage']['value'])) {
					if (isset($data['detailimage']['delete'])) {
						$data['post']['detailimage'] = null;
					} else if (isset($data['detailimage']['value'])) {
						$data['post']['detailimage'] = $data['detailimage']['value'];
					} else {
						$data['post']['detailimage'] = null;
					}
				}
