...
class Edit extends \Magento\Backend\App\Action
{
    ...
    protected function _initAction()
    {
        ...
        $resultPage->setActiveMenu('FME_StorePickup::StorePickup_holiday')
    ...
    public function execute()
    {
        ...
        if ($id) {
            ...
            if (!$this->model->getId()) {
                $this->messageManager
                ->addError(__('This holiday no longer exists.'));
                $resultRedirect = $this->resultRedirectFactory->create();
                return $resultRedirect->setPath('*/*/');
            }
        }

        $this->_coreRegistry->register('StorePickup_holiday', $this->model);

        $resultPage = $this->_initAction();

        $resultPage->addBreadcrumb(
            $id ? __('Edit Holidays') : __('New Holiday'),
            $id ? __('Edit Holidays') : __('New Holiday')
        );
        $resultPage->getConfig()->getTitle()->prepend(__('Holidays'));
        $resultPage->getConfig()->getTitle()
            ->prepend($this->model->getId() ? $this->model->getHolidayName() : __('New Holiday'));
