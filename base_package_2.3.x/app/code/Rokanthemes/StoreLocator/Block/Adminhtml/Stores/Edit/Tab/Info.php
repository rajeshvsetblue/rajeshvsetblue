...
class Info extends Generic
{
    ...
    protected function _prepareForm()
    {
		...
		if(isset($data['image_store'])){
			...
			if($image){
				$after_element_html = '<div style="margin-top:15px;"><a href="'.$image.'" target="_blank"><image src="'.$image.'" style="max-width:250px;" /></a><div>';
			}
		}
		$fieldset->addField(
            'image_stored',
			...
			$selectField->setAfterElementHtml('
				<script>
					require([
						...
						$(".sturday .admin__select-time").val('.$time->sturday_time.');
						
						$(".sturday .admin__field-control-to .from-hours").val('.$time->sturday->from->hours.');
						$(".sturday .admin__field-control-to .from-minutes").val('.$time->sturday->from->minutes.');
						$(".sturday .admin__field-control-to .to-hours").val('.$time->sturday->to->hours.');
						$(".sturday .admin__field-control-to .to-minutes").val('.$time->sturday->to->minutes.');
